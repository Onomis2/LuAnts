...
function DrawMenu()
    ...
    love.graphics.setColor(0.3, 0.2, 0.06)
...
function Play()
    Gamestate = "game"
    SpawnFood(tonumber(WindowWidth / 2 + (math.random(-150, 150))), tonumber(WindowHeight / 2 + (math.random(-150, 150))),
        20, nil)
end
...
function SetButtons()
    ...
    interactions = {
        ...
        options = {
            ...
            {
                func = function()
                    if resCounter == #resolutions then resCounter = 1 else resCounter = resCounter + 1 end
                    config.width = resolutions[resCounter].width
                    config.height = resolutions[resCounter].height
                    SetButtons()
                end,
            ...
            {
                func = function() if config.fullscreen then
            ...
            {
                func = function()
                    if monCounter == love.window.getDisplayCount() then monCounter = 1 else
                        monCounter = monCounter + 1
                        config.display = monCounter
                    end
                    SetButtons()
                end,
    ...
    if config.fullscreen then
        buttons.options[4].text = "Screen mode: Fullscreen"
    elseif config.fullscreen == false then
        buttons.options[4].text = "Screen mode: Windowed"
    end
end
